...
namespace
{
  ...
  {
      // 使用 QString 从 UTF-8 编码的字符数组中构建字符串
      QString temp = QString::fromUtf8(s, static_cast<int>(length));

      // 将 QString 转换为 std::wstring
      data = temp.toStdWString();
  }
...
ByteVector String::data(Type t) const
{
  ...
  case UTF8:
    {
        // 使用 toWString() 获取 std::wstring
        std::wstring wstr = toWString();
        QString temp = QString::fromStdWString(wstr);
        QByteArray utf8Data = temp.toUtf8();

        // 将 QByteArray 数据复制到 ByteVector
        ByteVector v(utf8Data.size(), 0);
        std::memcpy(v.data(), utf8Data.data(), utf8Data.size());
