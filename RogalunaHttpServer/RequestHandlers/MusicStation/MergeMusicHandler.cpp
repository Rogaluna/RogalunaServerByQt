...
QHttpServerResponse MergeMusicHandler::handleRequest(const QHttpServerRequest &request)
{
    ...
    QString fileUid = RogalunaHttpConfig::getInstance().getMusicServer()->mergeChunks(uid, fileName, totalChunks, userId.toInt());
    if(fileUid.isEmpty()) {
