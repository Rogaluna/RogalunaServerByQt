#include "UpdateBookReadProgressHandler.h"
...
QHttpServerResponse UpdateBookReadProgressHandler::handleRequest(const QHttpServerRequest &request)
{
    ...
    for (const MultipartPart &part : formParts) {
        if (part.name == "id") {
            ...
            percentProgress = QString::fromUtf8(part.data).trimmed();
        }
    }

    // 检查是否收到了必须的参数
    if (bookId.isEmpty() || chapterIndex.isEmpty() || percentProgress.isEmpty()) {
    ...
    // 更新进度数据
    bool bSuccess = RogalunaHttpConfig::getInstance().getLibraryServer()->updateBookReadProgress(bookId, userId, chapterIndex, percentProgress);
