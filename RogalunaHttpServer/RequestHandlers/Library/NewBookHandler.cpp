...
QHttpServerResponse NewBookHandler::handleRequest(const QHttpServerRequest &request)
{
    ...
    for (const MultipartPart &part : formParts) {
        if (part.name == "name") {
            bookName = QString::fromUtf8(part.data).trimmed();
    ...
    // 新建书籍，写入数据库
    QString bookId = RogalunaHttpConfig::getInstance().getLibraryServer()->registerBook(userId, bookName, bookDesc, tags);
