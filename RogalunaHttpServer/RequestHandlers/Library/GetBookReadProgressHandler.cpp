#include "GetBookReadProgressHandler.h"
...
QHttpServerResponse GetBookReadProgressHandler::handleRequest(const QHttpServerRequest &request)
{
    ...
    QJsonObject result = RogalunaHttpConfig::getInstance().getLibraryServer()->getBookReadProgress(bookId, userId);

    if (result.isEmpty()) {
        // 没有找到阅读进度记录，有可能是查询错误，但正常来说是用户没有阅读
        result["message"] = "Can't fetch read record.";
